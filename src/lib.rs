...
impl PRFC2Implementor {
    ...
    fn init(){
        ...
        let token = Token {
            id: "1".to_string(),
            name: "Belgian Coffee infused with orange zest".to_string(),
            metadata: "".to_string(),
            uri: "https://www.tastingtable.com/img/gallery/coffee-brands-ranked-from-worst-to-best/l-intro-1645231221.jpg".to_string(),
            owner: pchain_types::Base64URL::decode(&"qrtfMBVFj7uwHCtIVfoNEcDN917-vMGy-N7h5iTdEl8".to_string()).unwrap().try_into().unwrap(),
            spender: pchain_types::Base64URL::decode(&"qrtfMBVFj7uwHCtIVfoNEcDN917-vMGy-N7h5iTdEl8".to_string()).unwrap().try_into().unwrap(),
            exclusive_spender: pchain_types::Base64URL::decode(&"qrtfMBVFj7uwHCtIVfoNEcDN917-vMGy-N7h5iTdEl8".to_string()).unwrap().try_into().unwrap()
        };
        tokens.insert(&"1".to_string(), token);
        let collection = Collection{
            name:"Sid's Coffee Collection".to_string(),
            symbol:"SidCoffee".to_string(),
            tokens:tokens,
            uri:"".to_string()
        };
        PRFC2Implementor{
